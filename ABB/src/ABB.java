...
public class ABB<K, V> implements IMapeamento<K, V> {
     ...
     * Se o comparador fornecido for {@code null}, o comparador padrão de ordem
     * natural
     * será utilizado.
     * 
    ...
    public int inserir(K chave, V item) {
        /// Chama o método recursivo "inserir", responsável por adicionar, o item
        /// passado como parâmetro, à árvore.
        /// O método "inserir" recursivo receberá, como primeiro parâmetro, a raiz atual
        /// da árvore;
        /// como segundo parâmetro, a chave do item que será adicionado à árvore; e como
        /// terceiro parâmetro, o item.
        /// Por fim, a raiz atual da árvore é atualizada, com a raiz retornada pelo
        /// método "inserir" recursivo.
     ...
     * 
     * @param raizArvore a raiz da árvore ou sub-árvore em que o item será
     *                   adicionado.
     ...
     *         adicionado.
     * @throws RuntimeException se um item com a mesma chave já estiver presente na
     *                          árvore.
     */
    ...
    public V remover(K chave) {
        ...
        /// pesquisar o item que apresenta a chave passada como parâmetro na árvore e
        /// retirá-lo da árvore.
        /// O método "remover" recursivo receberá, como primeiro parâmetro, a raiz atual
        /// da árvore;
        /// e, como segundo parâmetro, a chave do item que deverá ser localizado e
     ...
     * árvore.
     * 
     * @param raizArvore   a raiz da árvore ou sub-árvore da qual o item será
     *                     retirado.
     * @param chaveRemover a chave do item que deverá ser localizado e removido da
     *                     árvore.
    ...
    protected No<K, V> remover(No<K, V> raizArvore, K chaveRemover) {
        ...
        if (comparacao == 0) {
            ...
            if (raizArvore.getDireita() == null) {
                ...
                /// O nó que está sendo retirado da árvore possui descendentes à esquerda e à
                /// direita.
    ...
    private String caminhamentoDecrescente(No<K, V> raizArvore) {
        if (raizArvore != null) {
            ...
            resposta += raizArvore.getItem().toString() + "\n";
            return resposta += caminhamentoDecrescente(raizArvore.getEsquerda());
        } else
            return "";
    }
    ...
    public ABB<K, V> clone() {
        if (vazia())
            throw new IllegalStateException("Árvore está vazia");
        ABB<K, V> arvore = new ABB<>(this.comparador);
        arvore.raiz = clone(this.raiz);
        arvore.tamanho = this.tamanho;
        return arvore;
    }

    private No<K, V> clone(No<K, V> raizArvore) {
        if (raizArvore != null) {
            No<K, V> novoNo = raizArvore.clone();
            novoNo.setEsquerda(clone(raizArvore.getEsquerda()));
            novoNo.setDireita(clone(raizArvore.getDireita()));
            return novoNo;
        }
        return null;
    }
    ...
    public boolean ehRaiz(K chave) {
        if (vazia())
            throw new IllegalStateException("Árvore está vazia");
        if (comparador.compare(chave, raiz.getChave()) == 0)
            return true;
        return false;
    }
    ...
    private V obterAntecessorRecursivo(K chave, No<K, V> raizArvore, No<K, V> antecessor) {
        ...
        if (comparacao > 0) {
            return obterAntecessorRecursivo(chave, raizArvore.getDireita(), raizArvore);
        } else if (comparacao < 0) {
            return obterAntecessorRecursivo(chave, raizArvore.getEsquerda(), antecessor);
        } else if (comparacao == 1) {
            if (raizArvore.getEsquerda() != null) {
                No<K, V> temp = raizArvore.getEsquerda();
                while (temp.getDireita() != null) {
                    temp = temp.getDireita();
                }
                return temp.getItem();
            }
            return antecessor != null ? antecessor.getItem() : null;
        } else {
    ...
    public Double calcularValorMedio(Function<V, Double> extrator) {
        ...
        SomaContador resposta = calcularValorMedio(extrator, raiz);
        return (resposta.soma / resposta.contador);

    }

    private SomaContador calcularValorMedio(Function<V, Double> extrator, No<K, V> raizArvore) {
        if (raizArvore == null)
            return new SomaContador(0.00, 0);

        SomaContador somaEsquerda = calcularValorMedio(extrator, raizArvore.getEsquerda());

        Double valor = extrator.apply(raizArvore.getItem());

        SomaContador somaDireita = calcularValorMedio(extrator, raizArvore.getDireita());

        return new SomaContador((somaEsquerda.soma + valor + somaDireita.soma),
                (somaEsquerda.contador + 1 + somaDireita.contador));
    }

    private static class SomaContador {
        double soma;
        int contador;

        SomaContador(double soma, int contador) {
            this.soma = soma;
    ...
    public int contarSe(Predicate<V> condicional, No<K, V> raizArvore) {
        if (raizArvore == null)
            return (0);

        int somaEsquerda = contarSe(condicional, raizArvore.getEsquerda());

        int valor = 0;
        if (condicional.test(raizArvore.getItem()))
            valor++;

        int somaDireita = contarSe(condicional, raizArvore.getDireita());

        return (somaEsquerda + valor + somaDireita);
    }
}